...
SELECT
  STR_TO_DATE(FullDate, '%Y-%m-%d %H:%i:%s') AS FullDate, -- Convert string to date and time format
...
FROM temp
WHERE STR_TO_DATE(FullDate, '%Y-%m-%d %H:%i:%s') IS NOT NULL; -- Ensure only valid date entries are inserted
